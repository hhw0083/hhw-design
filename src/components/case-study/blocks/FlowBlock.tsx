...
export function FlowBlock({ block }: { block: FlowContentBlock }) {
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div className="grid grid-cols-2 items-stretch gap-3 md:grid-cols-2 xl:grid-cols-7">
        {block.items.map((step, index) => (
          <div key={step} className="relative">
            ...
            {index < block.items.length - 1 ? (
              <ArrowRight
                className="mx-auto my-2 hidden size-4 text-cyanline sm:block xl:absolute xl:-right-3 xl:top-1/2 xl:my-0 xl:-translate-y-1/2"
                aria-hidden="true"
              />
            ) : null}
          </div>
